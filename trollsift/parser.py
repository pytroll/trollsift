...
class RegexFormatter(string.Formatter):
    ...
    >>> from trollsift.parser import RegexFormatter, extract_values
    ...
    >>> extract_values('{field_one:5d}_{field_two}', '12345_sometext')
    ...
    >>> extract_values('{field_one}_{field_two}', 'abc_def_ghi')
...
def _gen_data_for_spec(format_spec: str | None) -> int | str | dt.datetime:
    ...
    if format_spec and "d" in format_spec:
        ...
        return random.randint(0, 99999999999999999) % (10**n)  # noqa: S311
    ...
    randstri = ""
    for _ in range(n):
        randstri += random.choice(string.ascii_letters)  # noqa: S311
    return randstri
